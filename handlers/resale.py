...
_STEPS = [
    ...
    ("category", "📂 Категорія (напр. `кросівки`), або «немає»:", "text"),
    ("min_price", "💰 Мінімальна ціна купівлі (грн), або «немає»:", "num"),
    ("max_price", "💰 Максимальна ціна купівлі (грн), або «немає»:", "num"),
...
def _ikb_resale_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔎 Мої автопошуки", callback_data="rsm_list")],
        [InlineKeyboardButton(text="⏸ Поставити на паузу", callback_data="rsm_pause_all"),
         InlineKeyboardButton(text="▶️ Запустити", callback_data="rsm_resume_all")],
        [InlineKeyboardButton(text="⚙️ Налаштування", callback_data="rsm_settings"),
         InlineKeyboardButton(text="📊 Історія результатів", callback_data="rsm_history")],
...
def _ikb_settings() -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=f"{label} — змінити", callback_data=f"rsm_set:{key}")]
        for key, (label, _unit) in _SETTING_LABELS.items()
    ]
...
async def resale_monitor_stats_cb(cb: CallbackQuery):
    ...
    await cb.message.answer(
        f"📊 *Статистика «{esc(monitor.get('name') or monitor.get('category'))}»*\n\n"
        f"📅 Сьогодні: проаналізовано {day['found']}, відібрано {day['selected']}\n"
        f"🕒 Останній запуск: переглянуто {last.get('scanned', 0)}, "
        f"проаналізовано {last.get('analyzed', 0)}, відібрано {last.get('selected', 0)}\n\n"
        f"🔎 Показано у звітах всього: {stats.get('found', 0)}\n"
    ...
    for c in items:
        ...
        if c.get("shown_at"):
            try:
                ...
                pass
        lines.append(
            f"• {esc(c.get('title') or '—')[:60]} — {resale_service._money(c.get('price'))} грн "
            f"→ прибуток ~{resale_service._money(c.get('profit'))} грн {when}"
        )
    await cb.message.answer("\n".join(lines))
