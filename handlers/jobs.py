...
from keyboards.jobs import ikb_vacancy_card, ikb_not_interested_reasons, ikb_filters_menu, ikb_saved_item, ikb_watch_item, ikb_empty_search, ikb_apply_review, ikb_apply_final_confirm, ikb_apply_manual, ikb_autosearch_list_header, ikb_autosearch_remote, ikb_autosearch_level, ikb_autosearch_confirm
...
_MD_SPECIAL_RE = re.compile('([_*`\\[])')

def _md_escape(value) -> str:
    if not value:
        return ''
    return _MD_SPECIAL_RE.sub('\\\\\\1', str(value))
...
async def _run_search(msg: Message, uid: int, query_text: str):
    try:
        ...
        try:
            await msg.answer('⚠️ Сталася помилка під час пошуку вакансій. Спробуй ще раз трохи пізніше.', reply_markup=kb_main())
        except Exception:
...
async def _run_search_inner(msg: Message, uid: int, query_text: str):
    ...
    if not vacancies:
        ...
        return await _safe_edit(wait_msg, "📭 Нічого не знайшов за цим запитом на Djinni / Work.ua / Robota.ua.\n\nСпробуй ширші критерії (менше уточнень одразу) — або збережи цей пошук, і я сам повідомлю, щойно з'явиться щось підходяще.", reply_markup=ikb_empty_search())
    ...
    if not matched:
        ...
        if unscored == len(scored):
            text = f'⚠️ Знайшов {len(scored)} вакансій, але не вдалося оцінити їх відповідність профілю. Спробуй ще раз трохи пізніше.'
        else:
            text = f'📭 Знайшов {len(scored)} вакансій, але жодна не підходить більш ніж на {JOB_MIN_MATCH_PERCENT}% з твоїм профілем' + (f' (ще {unscored} не вдалося оцінити)' if unscored else '') + ".\n\nСпробуй ширший запит, доповни «👤 Мої дані для пошуку» — або збережи цей пошук, і я сам повідомлю, коли з'явиться щось підходяще."
        return await _safe_edit(wait_msg, text, reply_markup=ikb_empty_search())
    _results_cache[uid] = matched
    _position_cache[uid] = 0
    top3 = '\n\n'.join((f'{i + 1}. {_md_escape(v.get('title', ''))} — {_fmt_match_line(v['_score'])}\n🔗 {v.get('url', '')}' for i, v in enumerate(matched[:3])))
    ...
    if not _profile_ready(profile):
        await msg.answer(f'💡 Заповни «👤 Мої дані для пошуку» — і я зможу оцінювати відповідність вакансій, показувати лише ті, що підходять більш ніж на {JOB_MIN_MATCH_PERCENT}%, та писати персональні cover letter.')
...
@router.message(F.text.regexp('^\\s*шукаю\\b', flags=re.IGNORECASE))
async def jobs_natural_shukau(msg: Message, state: FSMContext):
    if not await require_auth(msg, state):
        return
    await _run_search(msg, msg.from_user.id, msg.text.strip())

@router.message(F.text.regexp('знайди.*(вакансі|роботу)', flags=re.IGNORECASE))
...
async def jobs_cover_cb(cb: CallbackQuery):
    ...
    if not _profile_ready(profile):
        await cb.answer()
        return await cb.message.answer('⚠️ Спочатку заповни «👤 Мої дані для пошуку» — cover letter пишеться на основі цих даних.')
    ...
    if not letter:
        return await cb.message.answer(AI_ERROR_TEXT)
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='🔄 Перегенерувати', callback_data=f'jb_cover:{idx}'), InlineKeyboardButton(text='📋 Скопіювати', callback_data=f'jb_copy_noop')]])
...
async def jobs_watch_empty_cb(cb: CallbackQuery):
    ...
    await cb.answer("Додано до моніторингу 🔔 Повідомлю, щойно з'являться нові вакансії за цим запитом.", show_alert=True)
...
async def jobs_watches_list(msg: Message, state: FSMContext):
    ...
    intro = '🔔 *Мої автопошуки вакансій*\n\nКожен автопошук самостійно шукає вакансії ДВІЧІ на день (в обід і ввечері) і надсилає підсумок увечері.' if watches else '📭 Ще немає жодного автопошуку.\n\nСтвори перший — і я сам шукатиму для тебе двічі на день і пришлю підсумок увечері.'
...
async def jobs_stats(msg: Message, state: FSMContext):
    ...
    lines = ['📊 *Мій пошук роботи*', '', f'⭐ Збережено: {stats['total_saved']}', f'📨 Відгукнувся: {stats['applied']}', f'💬 Відповідей: {stats['response']}', f'🎤 Співбесід: {stats['interview']}', f'✅ Прийнято: {stats['hired']}', f'❌ Відмов: {stats['rejected']}']
...
def _fmt_apply_review(vacancy: dict, cover_letter: str) -> str:
    ...
    lines = [f'💼 *{_md_escape(vacancy.get('title', ''))}*', f'🏢 {_md_escape(vacancy.get('company')) or '—'}']
    if score.get('match_percent') is not None:
...
async def _show_apply_review(target: Message, idx: int, vacancy: dict, cover_letter: str) -> None:
    await _answer_safe(target, _fmt_apply_review(vacancy, cover_letter), reply_markup=ikb_apply_review(idx))
...
async def jobs_apply_start_cb(cb: CallbackQuery):
    ...
    if not _profile_ready(profile):
        await cb.answer()
        return await cb.message.answer('⚠️ Спочатку заповни «👤 Мої дані для пошуку» — без цього я не зможу підготувати персональний Cover Letter для відгуку.')
    if not jobs_service.ai_service.is_available():
        await cb.answer()
        return await cb.message.answer(AI_ERROR_TEXT)
    await cb.answer()
...
async def jobs_apply_edit_start_cb(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer('✏️ Надішли новий текст Cover Letter (повністю, він замінить поточний):', reply_markup=kb_cancel())
...
async def jobs_apply_send_confirm_cb(cb: CallbackQuery):
    ...
    await cb.answer()
    await cb.message.answer(f'❓ *Готово до відправлення?*\n\nВакансія: {_md_escape(v.get('title', ''))}\nКомпанія: {_md_escape(v.get('company')) or '—'}\n\nВідправити заявку?', reply_markup=ikb_apply_final_confirm(idx))
...
async def jobs_apply_send_final_cb(cb: CallbackQuery):
    ...
    if result.get('success'):
        ...
        return await _safe_edit(wait, f'✅ Заявку відправлено автоматично!\n💼 {_md_escape(v.get('title', ''))}')
    await _safe_edit(wait, 'ℹ️ Автоматична подача заявки на цьому сайті наразі технічно неможлива (немає публічного API / форма захищена від ботів).\n\nВідкрий вакансію за посиланням нижче, скопіюй Cover Letter і надішли заявку вручну. Коли зробиш це — натисни «Я відгукнувся вручну».')
    await cb.message.answer(f'✉️ *Cover Letter для копіювання:*\n\n{_md_escape(letter)}', reply_markup=ikb_apply_manual(idx, v.get('url', '')))
...
async def autosearch_new_cb(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer('➕ *Новий автопошук*\n\nПридумай коротку назву (наприклад: «Frontend Developer», «Remote / Online робота», «Junior Developer», «Робота в Польщі»):', reply_markup=kb_cancel())
...
async def autosearch_level_cb(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.answer()
    await cb.message.answer('Мінімальна бажана зарплата? Напиши число (напр. 30000 або 1500$) або «пропустити»:', reply_markup=kb_cancel())
...
async def autosearch_salary_received(msg: Message, state: FSMContext):
    ...
    summary = f'📋 *Перевір автопошук:*\n\n📝 Назва: {_md_escape(data['name'])}\n💼 Посада/ключові слова: {_md_escape(data['position'])}\n📍 Локація: {_md_escape(data.get('city') or 'будь-де')}\n💻 Формат: {format_labels.get(data.get('work_format', 'any'), 'не важливо')}\n📊 Досвід: {level_labels.get(data.get('level', 'any'), 'будь-який')}\n💰 Зарплата від: {(salary_min if salary_min else 'не вказано')} {(salary_currency if salary_min else '')}\n\nАвтопошук працюватиме двічі на день і надсилатиме підсумок увечері (лише вакансії, що підходять більш ніж на {JOB_MIN_MATCH_PERCENT}%).'
...
async def autosearch_confirm_cb(cb: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await cb.answer()
    await cb.message.answer(f'✅ Автопошук «{_md_escape(name)}» створено! Шукатиму двічі на день і надсилатиму підсумок увечері.', reply_markup=kb_main())
...
async def send_autosearch_digest(bot: Bot, uid: int, searches: list[dict]) -> None:
    lines = ['🌙 *Вечірній підсумок автопошуку вакансій*', f'_Лише вакансії, що підходять більш ніж на {JOB_MIN_MATCH_PERCENT}%_', '']
    ...
    if not any_found:
        lines.append(f'\nСьогодні по жодному з твоїх автопошуків не знайшлось вакансій зі збігом понад {JOB_MIN_MATCH_PERCENT}% 😔')
    text = '\n'.join(lines)
    try:
