...
_JUNK_RE = re.compile('запчастин|на запчаст|не працю[єе]|розбит|нероб[оа]ч|на розбір|під відновлення|заблокован|icloud', re.IGNORECASE)
_BAD_COND_RE = re.compile('не працю|несправн|зламан|розбит|на запчастин|запчастин|під відновлення|заблокован', re.IGNORECASE)

def esc(text) -> str:
    return re.sub('([_*`\\[])', '\\\\\\1', str(text if text is not None else ''))
...
def build_opportunity(listing: dict, analysis: dict, monitor: dict, market_median: float | None, market_n: int, us: dict) -> tuple[dict | None, str | None]:
    ...
    score = 100 * (0.35 * profit_norm + 0.25 * margin_norm + 0.15 * discount_norm + 0.1 * demand + 0.08 * cond_q + 0.07 * speed)
    ...
    return ({'price': price, 'est_sale': round(est), 'profit': profit, 'margin': margin, 'costs': costs, 'score': round(score, 1), 'market_median': round(market_median) if market_median else None, 'market_n': market_n, 'market_verified': market_verified, 'why': why}, None)
...
async def _scan(monitor: dict, uid: int, mid: str, res: dict):
    ...
    for r, details in fetched:
        ...
        if await ai_usage_db.get_remaining(uid, AI_DAILY_LIMIT) <= 0:
            ...
            break
        listing = {'source': domain, 'url': url, 'title': details.get('title') or r.get('title'), 'price': price, 'currency': details.get('currency', r.get('currency', 'UAH')), 'description': details.get('description'), 'location_text': details.get('location_text') or r.get('location_text'), 'views': details.get('views'), 'photos': details.get('photos') or [], 'photos_count': details.get('photos_count'), 'params': details.get('params') or []}
        try:
        ...
        if opp:
            ...
            await resale_db.upsert_candidate(mid, uid, url, {**opp, 'title': listing['title'], 'currency': listing['currency'], 'condition_text': _short(str(analysis.get('item_condition') or ''), 50), 'listing': _slim_listing(listing), 'analysis': analysis, 'status': 'new', 'day': day, 'found_at': now, 'price_drop_from': drops.get(url), 'recheck_fail': 0})
            res['selected'] += 1
        else:
            await resale_db.upsert_candidate(mid, uid, url, {'status': 'rejected', 'reject_reason': reason, 'price': price, 'title': listing['title'], 'day': day})
            res['rejected'] += 1
...
async def recheck_candidates(monitor: dict, limit: int=12):
    ...
    for c, fetched in await asyncio.gather(*[_one(c) for c in cands]):
        ...
        opp, reason = build_opportunity(listing, c.get('analysis') or {}, monitor, c.get('market_median'), c.get('market_n') or 0, us)
        if opp:
            await resale_db.update_candidate(c['_id'], {**opp, 'listing': listing, 'recheck_fail': 0, 'price_changed_from': c.get('price')})
        else:
            await resale_db.update_candidate(c['_id'], {'status': 'rejected', 'reject_reason': f'price_changed:{reason}', 'price': new_price})
...
def opp_from_candidate(c: dict) -> dict:
    ...
    return {'monitor_id': c.get('monitor_id'), 'listing': listing, 'analysis': c.get('analysis') or {}, 'score': c.get('score'), 'margin': c.get('margin'), 'profit': c.get('profit')}
...
def ikb_report(items: list[dict]) -> InlineKeyboardMarkup:
    ...
    for i, c in enumerate(items, 1):
        ...
        rows.append([InlineKeyboardButton(text=f'🤖 Аналіз {i}', callback_data=f'rso_analyze:{cid}'), InlineKeyboardButton(text=f'⭐ Зберегти {i}', callback_data=f'rso_save:{cid}'), InlineKeyboardButton(text=f'❌ {i}', callback_data=f'rso_skip:{cid}')])
...
def format_scan_summary(res: dict, first: bool=False) -> str:
    ...
    text = f'✅ {('Перший збір' if first else 'Збір')} завершено: на OLX переглянуто {res.get('scanned', 0)}, проаналізовано AI {res.get('analyzed', 0)}, відібрано {res.get('selected', 0)}.\nЗвіт надійде ввечері (~{eh:02d}:{em:02d}).'
...
def format_monitor_card(m: dict, day: dict) -> str:
    ...
    lines = [f'🔥 *{esc(m.get('name') or m.get('category') or '—')}* — {status}', f'{price}, {esc(loc)}']
    if crit:
        lines.append(', '.join(crit))
    lines += ['', f'🔎 Знайдено оголошень (останній запуск): {last.get('scanned', 0)}', f'🎯 Відібрано (останній запуск): {last.get('selected', 0)}', f'📅 Можливостей за день: {day.get('selected', 0)} (проаналізовано {day.get('found', 0)})', f'💵 Найкращий прибуток сьогодні: {('~' + _money(best) + ' грн' if best is not None else '—')}', f'🕒 Останній запуск: {_fmt_dt(m.get('last_checked_at'))}', f'⏭ Наступний: {(next_run_text() if active else '— (на паузі)')}']
    return '\n'.join(lines)
...
def build_statistics_text(monitors: list[dict], saved: list[dict]) -> str:
    ...
    lines = ['📈 *Статистика перепродажу*', '', f'🔎 Показано можливостей у звітах: {found}', f'⭐ Збережено: {saved_n}', f'🛒 Куплено: {bought}', f'💰 Перепродано: {sold}', f'💵 Потенційний прибуток: ~{potential_profit:.0f} грн', f'✅ Фактичний прибуток: ~{actual_profit:.0f} грн']
