...
async def add_monitor(uid: int, params: dict) -> str:
    ...
    doc = {
        ...
        "min_resale_price": params.get("min_resale_price"),
...
async def set_monitor_status(monitor_id, uid: int, status: str) -> bool:
    ...
    if oid is None:
        return False
    result = await db_call(
        resale_monitors_col.update_one({"_id": oid, "uid": uid}, {"$set": {"status": status}})
    )
    return result.matched_count > 0
...
async def set_all_status(uid: int, status: str) -> int:
    result = await db_call(
        resale_monitors_col.update_many({"uid": uid}, {"$set": {"status": status}})
    )
...
async def set_run_info(monitor_id, info: dict):
    ...
    await db_call(
        resale_monitors_col.update_one(
            {"_id": oid},
            {"$set": {"last_checked_at": now, "last_run": {**info, "at": now}}},
        ),
        raise_on_fail=False,
    )
...
async def increment_stat(monitor_id, field: str, amount: float = 1):
    ...
    if oid is None:
        return
    await db_call(
        resale_monitors_col.update_one({"_id": oid}, {"$inc": {f"stats.{field}": amount}}),
        raise_on_fail=False,
    )
...
async def mark_shown(monitor_id, items: list[dict]):
    ...
    if urls:
        await db_call(
            resale_monitors_col.update_one({"_id": oid}, {"$pull": {"shown": {"url": {"$in": urls}}}}),
            raise_on_fail=False,
        )
        await db_call(
            resale_monitors_col.update_one(
                {"_id": oid},
                {"$push": {"shown": {"$each": entries, "$slice": -MAX_SHOWN_PER_MONITOR}}},
            ),
            raise_on_fail=False,
        )
    ...
    if ids:
        await db_call(
            resale_candidates_col.update_many(
                {"_id": {"$in": ids}},
...
async def add_learned_keyword(monitor_id, keyword: str, positive: bool):
    ...
    await db_call(
        resale_monitors_col.update_one({"_id": monitor["_id"]}, {"$set": {field: lst}}),
        raise_on_fail=False,
    )
...
async def add_blocked_similar(monitor_id, keyword: str):
    ...
    if keyword not in lst:
        lst.append(keyword)
    await db_call(
        resale_monitors_col.update_one({"_id": monitor["_id"]}, {"$set": {"blocked_similar": lst}}),
        raise_on_fail=False,
    )
...
async def upsert_candidate(monitor_id, uid: int, url: str, fields: dict):
    ...
    await db_call(
        resale_candidates_col.update_one(
            {"monitor_id": mid, "url": url},
...
async def update_candidate(cid, fields: dict):
    ...
    await db_call(
        resale_candidates_col.update_one(
            {"_id": oid}, {"$set": {**fields, "updated_at": datetime.now().isoformat()}}
        ),
        raise_on_fail=False,
    )
...
async def day_stats(monitor_id, day: str) -> dict:
    mid = str(monitor_id)
    found = await db_call(
        resale_candidates_col.count_documents({"monitor_id": mid, "day": day}), raise_on_fail=False
    ) or 0
    ...
    best = await db_call(
        resale_candidates_col.find_one(sel_q, sort=[("profit", -1)]), raise_on_fail=False
    )
    return {
        "found": found,
        "selected": selected,
        "best_profit": (best or {}).get("profit"),
    }


async def get_history(uid: int, limit: int = 10) -> list[dict]:
    cursor = (
        resale_candidates_col.find({"uid": uid, "status": "shown"})
        .sort("shown_at", -1)
        .limit(limit)
    )
    return await db_call(cursor.to_list(length=limit), raise_on_fail=False) or []
...
async def set_user_setting(uid: int, key: str, value: float):
    if key not in _default_user_settings():
        return
    await db_call(
        resale_settings_col.update_one({"uid": uid}, {"$set": {key: float(value)}}, upsert=True),
        raise_on_fail=False,
    )
